...
fn test-simple {
    var res = (echo (riku:get "https://httpbin.org/get")[content] | from-json)
    var user_agent = $res[headers][User-Agent]
    put (test:check { eq $user_agent "riku/"$riku:version } "Simple GET request")
}

fn test-params {
    var res = (echo (riku:get "https://httpbin.org/get" &params=[&key=value])[content] | from-json)
...
fn test-cookie {
    var res = (riku:get "https://httpbin.org/cookies/set/keks/lecker")
...
fn test-headers {
    var res = (riku:get "https://httpbin.org/get" &headers=[&Authorization=password] | from-json)
    put (test:check { eq $res[args] [&key=value] } "GET request with headers")
}

var tests = (test:set get [
        (test-simple)
        (test-params)
        (test-200)
        (test-300)
