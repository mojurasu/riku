...
fn test-simple {
    var res = (echo (riku:post "https://httpbin.org/post")[content] | from-json)
    var user_agent = $res[headers][User-Agent]
    put (test:check { eq $user_agent "riku/"$riku:version } "Simple POST Request")
}

fn test-map-data {
    var res = (echo (riku:post "https://httpbin.org/post" &data=[&key="value"])[content] | from-json)
