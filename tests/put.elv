...
fn test-simple {
    var res = (echo (riku:put "https://httpbin.org/put")[content] | from-json)
    var user_agent = $res[headers][User-Agent]
    put (test:check { eq $user_agent "riku/"$riku:version } "Simple PUT Request")
}

fn test-map-data {
    var res = (echo (riku:put "https://httpbin.org/put" &data=[&key="value"])[content] | from-json)
